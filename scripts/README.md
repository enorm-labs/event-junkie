...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| `architecture-diagram.sh`                          | What each environment deploys, as sorted text, against the copy in `docs/architecture/` (ADR-034)                                          | `validate-chart.yml`, `/verify`                                              |
...
| `lighthouse.sh`                                    | Four Lighthouse cells against a deployed origin, three runs each; gates only the audits that are deterministic                             | `lighthouse.yml`                                                             |
...
| `seed-sources.py`            | Register the venues and event sources from `dev-seed.http` on a cluster; its dry run and `--site` report drift (#876, #1782)    | `docs/ops/CLUSTER_ACCESS.md` § 6a, `seed-drift.yml`     |
...
| `o2-query.sh`                | Read an environment's logs, Kubernetes events, alert history and metrics out of OpenObserve; `sweep` is what `/log-check` reads | `.github/prompts/log-check.prompt.md`                   |
