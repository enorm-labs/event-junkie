...
| 10a | **Object Storage S3 key, OpenObserve's own**                                      | Created 2026-08-20 so the observability stack does not hold #10. A pod that parses untrusted venue HTML should not carry a credential reaching the OpenTofu state and the database backups | The `openobserve-credentials` Secret on staging (`flux-system` **and** `observability`), and the password manager                                                                   | Yes                                   |
