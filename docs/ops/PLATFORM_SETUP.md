...
| 9   | **Traefik security headers + rate limiting**                             | A content security policy, HSTS, `X-Content-Type-Options`, frame options — below. The rate limit bounds concurrency; the per-source half waits on the client address surviving ServiceLB — [#268](https://github.com/enorm-labs/event-junkie/issues/268) |
...
**Item 9's limit is two limits, and only one of them can work here.** `inFlightReq` bounds concurrent requests and Traefik groups it by request host, so it
applies whatever address reaches the edge. `rateLimit` groups by the client's address — and k3s exposes Traefik through ServiceLB, whose `klipper-lb` container
masquerades every packet before Traefik reads it. Traefik's peer is the `svclb` pod for every visitor, and `X-Forwarded-For` carries the same address. A
per-source limit would group the whole internet as one source. It is therefore rendered but disabled, and the Traefik Service needs
`externalTrafficPolicy: Local` before it becomes true. The chart's README carries the values and the measurement behind them.
