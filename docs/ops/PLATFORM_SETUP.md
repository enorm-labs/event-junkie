...
flowchart LR
    ...
    flux -->|"repository_dispatch"| gha["Actions: deployment-status.yml, flux-source-failure.yml"]
...
Flux's **notification-controller** reports back through a `githubdispatch` Provider and two Alerts per cluster, in
...
| `importer.musicbrainz.enriched{field}`         | Counter                               | Fields filled from an `EXACT` entity: website … description; `error`, read failed  |
...
| 9   | **Traefik security headers + rate limiting**                             | A CSP, HSTS, nosniff, frame-deny, permissions, and cross-origin policies — below. The rate limit bounds concurrency and per-source request rate, both on since [#268](https://github.com/enorm-labs/event-junkie/issues/268) |
