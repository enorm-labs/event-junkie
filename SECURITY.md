...
This is a single-maintainer project, so there is no formal SLA. What you can expect:
...
The public site is <https://event-junkie.de>. Staging has no public address and is not a target.
...
- **Trivy** on every published container image, failing on a fixable HIGH or CRITICAL finding, and again nightly on the images that are deployed.
- **OWASP ZAP and Nuclei** nightly against the chart on a throwaway cluster, and weekly against the public site, where ZAP scans only passively.
- **gitleaks** as a pre-commit hook, and GitHub secret scanning with push protection, to keep credentials out of the history.
