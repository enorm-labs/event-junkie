...
through [`.claude/rules/`](.claude/rules), which holds one symlink per topic — `architecture`, `kotlin`, `logging`, `python`, `comments`, `design`, `documentation`, `markdown`, `vue`, `testing`, `kubernetes`, `ci-cd`. Each
