...
2. **Rule out what isn't actually available.** An issue is not pickable if it carries:
    ...
    - `needs-decision` — unless the _decision itself_ is the work you're recommending, which is often the right answer
...
- **"Nothing is ready" is a real answer.** If everything unblocked is `needs-decision`, the recommendation is to _make one of those decisions_, and say which
  unblocks the most.
