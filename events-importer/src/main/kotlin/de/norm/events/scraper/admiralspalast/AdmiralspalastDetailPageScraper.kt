 ...
 *
 * This is where the schedule lives. `#eventlist` renders one `.item` row **per performance**,
 * so a production playing four nights yields four events:
 *
 * ```html
 ...
 * ```
 *
 * The date is split across two spans with a German month abbreviation, so it is assembled, not
 * parsed as one string; the weekday span carries the start time after the comma.
 *
...
class AdmiralspalastDetailPageScraper {
     ...
     *
     * @param sourceUrl the production's URL: [ScrapedEvent.sourceUrl] and the slug half of each
     * [ScrapedEvent.sourceId].
     * @param category the category the venue files this production under, or `null` when it is on
     * no filter page. Drives [ScrapedEvent.eventType] only — see [parsePerformance] for why it is
     * not stored as the genre.
     */
    fun scrape(
        document: Document,
        sourceUrl: String,
    ...
    /**
     * Parses one `.item` performance row, or `null` without a title or usable date.
     *
     * The [category] drives [ScrapedEvent.eventType] and is deliberately **not** the genre. The
     * `eventkategorie` vocabulary names a staging format, not a musical style — `Konzert`, `Show`,
     * `Lesung`, `Podcast`, `Comedy`, `Tanz`, `Ballett`, `Diskussion`, `Kultur` — so filing it as
     * genre made 96 events read as genre-tagged while carrying no style ("Konzert" as the genre of
     * a concert) and pushed format labels into the genre-tag vocabulary. The category is already
     * the event *type*; the house publishes no genre, so none is stored.
     */
    ...
    private fun parsePerformance(
        ...
        return ScrapedEvent(
            ...
            sourceUrl = sourceUrl,
            // One production, many nights — and on a matinee day, two performances in one night's slot.
            // The date alone is no identity: this house plays *Mamma Mia* at 14:30 and 19:30 on the same
            // Saturday, each with its own ticket link, and both would arrive under one `sourceId`.
            // `event.source_id` is `UNIQUE`, so the second is dropped before the database
            // (EventUpsertService.deduplicateScrapedEvents) — the matinee never existed as far as the app
            // knew. Appending the start time makes each performance its own event; no colon so the id
            // reads as one token, matching the Uber and Heimathafen scrapers.
            sourceId =
                "${EventSource.ADMIRALSPALAST.sourceIdPrefix}$slug-$eventDate" +
                    startTime?.format(SOURCE_ID_TIME)?.let { "-$it" }.orEmpty(),
            // No lineup is billed, so the act is whatever the production title names. The subtitle is
            // not offered as a support-act source: it only ever carries the reschedule note, which would
            // be minted as a performer.
    ...
    /**
     * The site root a poster path is relative to. Contao writes every asset path relative
     * (`assets/images/8/…`) and declares a page-wide
     * `<base href="https://www.admiralspalast.theater/">` — but emits that tag **unterminated**,
     * so it cannot be trusted to survive parsing. Resolving against the root reproduces what the
     * tag intends; against the production URL it would yield `/veranstaltung/assets/…`, which 404s.
     */
    private fun siteRoot(url: String): String = URI(url).resolve("/").toString()

    /**
     * Assembles the performance date from the day and `MMM yyyy` spans rendered separately.
     */
    ...
    /**
     * The reschedule note, whose two idioms mean opposite things. `verschoben auf <date>` sits on
     * the **original** date, which is postponed. `verlegt vom <date>` sits on the **replacement**
     * date, going ahead as printed — the shared [parseEventStatus] reads any "verlegt" as
     * `RELOCATED`, which would mark the one date that is definitely happening as moved. Everything
     * else is left to the shared vocabulary.
     */
    ...
    private companion object {
        ...
        /** What the ticket cell says instead of a link once a performance has sold out. */
