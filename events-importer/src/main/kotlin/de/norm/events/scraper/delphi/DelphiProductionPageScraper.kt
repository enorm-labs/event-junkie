 ...
 *
 * @property description the full blurb, several paragraphs where the row carries a one-sentence teaser.
...
data class DelphiProduction(
    ...
    /**
     * Applies this production's data to one of its dates. The production page wins on both fields
     * it states — blurb over teaser, photo over thumbnail — and a field it leaves empty keeps
 ...
 * `table.program_table_schmal` and describes the show once. Only what the row cannot carry is
 * read — the full blurb and the full-width photo. Everything else (identity, date, clock, type,
 * price, ticket link) is settled by the programme page, which states it per performance.
 *
...
class DelphiProductionPageScraper {
    /**
     * Parses a production page, or `null` without a production heading — the marker that the
     * request did not return a real production page.
     */
