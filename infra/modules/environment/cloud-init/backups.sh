#!/usr/bin/env bash
#
# wal-g: continuous WAL archiving and daily base backups to Object Storage (#270). The volume
# (#460) survives the node, not a `DROP TABLE`.
#
# Commented thinly on purpose: this shares the 32 KiB `user_data` cap with every other script,
# and on the co-located node the cap binds. The reasoning is in infra/AGENTS.md § "Backups";
# re-measure after changing anything below.
#
# NO S3 CREDENTIAL LIVES HERE: `user_data` is state. The operator writes
# /etc/wal-g/credentials.env by hand (CLUSTER_BOOTSTRAP.md §8b), so a rebuilt node has timers and
# no credential, which `walg check` exists to catch. wal-g ships only as a GitHub release and
# github.com publishes no AAAA record, so a node with no public IPv4 stops the boot here.
...
# Both environments share the bucket; BACKUP_PREFIX is derived from `environment` and keeps
...
# One dispatcher, four verbs: `archive` is archive_command, `fetch` the restore_command, `backup`
# the nightly base backup and retention sweep, `check` the assertion that any of it happened.
# FIND_FULL and the 26-hour freshness bound are load-bearing (AGENTS.md § "Backups").
...
check)
    ...
    # A stalled archive_command fills pg_wal and stops the database, on a 10 GB volume.
    ...
    #
    # The \`else\` is the point of #518: without a URL the assertions above fail into
    # \`systemctl status walg-check\`, which nobody reads, and a dead-man's switch that is not wired
    # up reports exactly what a healthy one does. So say so, on every run, where somebody reads.
...
# archive_mode needs a restart, not a reload, which is why this is not in postgres.sh.
